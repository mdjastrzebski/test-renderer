...
- **`src/host-instance.ts`**: Defines `TestElement`, a wrapper around the internal fiber nodes with a DOM-like API (e.g., `children`, `props`, `parent`).
- **`src/render-to-json.ts`**: Handles the serialization of `TestElement` trees into JSON format for snapshots.
